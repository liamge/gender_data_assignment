...
if __name__ == "__main__":
    ...
    if not args['load']:
        ...
        for i in range(len(class_idx)):
            X, y = features, labels[i]
            rus = RandomUnderSampler()
            X_resampled, y_resampled = rus.fit_sample(X, y)
            trait = header[class_idx[i]]
            clf = svm.SVC().fit(X_resampled, y_resampled)
            #clf = svm.SVC(class_weight={1: sum(labels[i])/len(labels[i])}).fit(X, y)
            predicted = cross_val_predict(clf, X_resampled, y_resampled, cv=10)
            print("%s: %.2f" % (header[class_idx[i]], metrics.accuracy_score(y_resampled, predicted)))
