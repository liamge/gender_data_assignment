...
def predict_gender(X, Y, classifier='naive_bayes'):
    if classifier == 'random_forest':
        scores = cross_val_score(RandomForestClassifier(n_estimators=20, max_features=None, n_jobs=-1), X, Y, scoring='accuracy', cv=10)
    elif classifier == 'mlp':
